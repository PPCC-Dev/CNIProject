...
Public Class POReceiving
        ...
        If txtbarcode.Text <> "" Then
            ...
            If lblbarcode.Text = "Scan Delivery Sheet : " Then
                ...
                Else
                    ...
                    If lblWebDeliveryReceive.Text = "0" Then
                        lblbarcode.Text = "Scan Lines : "
                    End If
    ...
    Protected Sub btnDetail_Click(sender As Object, e As EventArgs) Handles btndetail.Click
        ...
        Response.Redirect("POTagDetail.aspx" & PostURL)
    ...
    Protected Sub PanelList_ItemDataBound(ByVal sender As Object, ByVal e As ListViewItemEventArgs) Handles PanelList.ItemDataBound
        ...
        If e.Item.ItemType = ListViewItemType.DataItem Then
            ...
            lblListMatlCost.Text = FormatNumber(MaterialCost.ToString, 4)
            lblListLineAmt.Text = FormatNumber(Math.Round(LineAmt, CInt(AmtTotalFormat.Value), MidpointRounding.AwayFromZero), 4)
...
#Region "Bind Data To Gridview"
    ...
    Sub BindListPOReceive()
        ...
        Filter = "DSNum = '" & lblBarcodeDSNum.Text & "' AND SessionID = '" & Session("PSession").ToString & "'"


        PropertyList = "InvDate, PoNum, Loc, Name, VendInv, Whse, TotalDSLine, Item, Description, QtyOrder, QtyReceived, QtyRemain, ReceiveQty, Lot, UnitMatCostConv, LineAmt, Places, UM, PoUM, "
        PropertyList = PropertyList + "DerInvoiceAmt, DerTotalInvoice, DerVAT, ReceiveQtyConv, Round, QtyDelivery"

        ds = oWS.LoadDataSet(Session("Token").ToString, "PPCC_EX_PO_Receives", PropertyList, Filter, "PoNum, PoLine, PoRelease", "", 0)
        ...
        If ds.Tables(0).Rows.Count > 0 Then
            ...
            If lblWebDeliveryReceive.Text = "0" Then
                ...
                txtdate.Text = DateTime.Parse(ds.Tables(0).Rows(0)("InvDate")).ToString("dd/MM/yyyy")
                txtPONo.Text = ds.Tables(0).Rows(0)("PoNum").ToString
                'txtLoc.Text = ds.Tables(0).Rows(0)("Loc").ToString
                txtvendname.Text = ds.Tables(0).Rows(0)("Name").ToString
                txtvendVendInv.Text = ds.Tables(0).Rows(0)("VendInv").ToString
                txtWhse.Text = ds.Tables(0).Rows(0)("Whse").ToString
                txtTotalLine.Text = ds.Tables(0).Rows(0)("TotalDSLine").ToString
                AmtTotalFormat.Value = ds.Tables(0).Rows(0)("Places").ToString
                lblInvoceAmt.Text = ds.Tables(0).Rows(0)("DerInvoiceAmt").ToString
                lblVAT.Text = ds.Tables(0).Rows(0)("DerVAT").ToString
                lblTotalInvoice.Text = ds.Tables(0).Rows(0)("DerTotalInvoice").ToString

                Decimal.TryParse(lblInvoceAmt.Text, InvoceAmt)
                Decimal.TryParse(lblVAT.Text, VAT)
                Decimal.TryParse(lblTotalInvoice.Text, TotalInvoice)
